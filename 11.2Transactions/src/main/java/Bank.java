    ...
    public void transfer(String fromAccountID, String toAccountID, long amount, boolean isBlocked) throws Exception
    {
        ...
        if (amount > MAX_AMOUNT){
            try {
                ...
                if (isFraud){
                    getAccount(fromAccountID).setIsBlocked(true);
                    getAccount(toAccountID).setIsBlocked(true);
                }
            ...
            if (getAccount(fromAccountID).withdraw(amount)){
                getAccount(toAccountID).replenish(amount);
            }
